...
def excel_input_old(driver,url,site_num):
    source_type = ['移動源 C1','固定燃燒源 C1','工業製程 C1','人為逸散 C1','其他關注類物質 C1','輸入電力 C2','輸入蒸汽 C2']

    file_path = 'C:\\Users\\iou85\\Desktop\\selenium資策會測試\\excel舊有模板\\'
    upload_path = ['Vehicle.xlsx','StationaryCombustionSource.xlsx','IndustrialProcess.xlsx','workhour.xlsx','RFG.xlsx','Fire.xlsx','other.xlsx','Elec.xlsx','GreenElec.xlsx','steam_plus.xlsx','steam_plus.xlsx']
    # source_type_files = {
    #                 '移動源 C1': 'Vehicle.xlsx',
    #                 '固定燃燒源 C1': 'StationaryCombustionSource.xlsx',
    #                 '工業製程 C1': 'IndustrialProcess.xlsx',
    #                 '人為逸散 C1(工時計算)': 'workhour.xlsx',
    #                 '人為逸散 C1(冷媒設備)': 'RFG.xlsx',
    #                 '人為逸散 C1(消防設備)': 'Fire.xlsx',
    #                 '其他關注類物質 C1': 'other.xlsx',
    #                 '輸入電力 C2(一般用電)': 'Elec.xlsx',
    #                 '輸入電力 C2(綠電)': 'GreenElec.xlsx',
    #                 '輸入蒸汽 C2(加項)': 'steam_plus.xlsx',
    #                 '輸入蒸汽 C2(減項)': 'steam_plus.xlsx',
    #                 '客戶和訪客運輸 C3': 'EmployeeCommuting.xlsx',
    #                 '員工差旅 C3': 'EmployeeCommuting.xlsx',
    #                 '員工通勤 C3': 'EmployeeCommuting.xlsx',
    #                 '營運產生之廢棄物 C4': 'LifecycleAssmt.xlsx',
    #                 '購買資本物品 C4': 'LifecycleAssmt.xlsx',
    #                 '輸入能源上游排放 C4(輸入電力上游)': 'LifecycleAssmt.xlsx',
    #                 '用水(水資源管理) B.5.2.a': 'LifecycleAssmt.xlsx',
    #                 '購買商品 C4': 'PurchasedProduct.xlsx',
    #                 '上游租賃資產 C4': 'LifecycleAssmt.xlsx',
    #                 '顧問諮詢、清潔、維護等 C4': 'LifecycleAssmt.xlsx',
    #                 '上游運輸及配送 C3': 'MaterialTransport.xlsx',
    #                 '廢棄物運輸 C3': 'MaterialTransport.xlsx',
    #                 '下游運輸及配送 C3': 'MaterialTransport.xlsx',
    #                 '下游租賃資產 C5': 'LifecycleAssmt.xlsx',
    #                 '產品壽命終止階段 C5': 'LifecycleAssmt.xlsx',
    #                 '產品使用階段 C5': 'LifecycleAssmt.xlsx',
    #                 '投資 C5': 'LifecycleAssmt.xlsx',
    #                 '其它間接排放 C6': 'LifecycleAssmt.xlsx'
    #                 }
                   
    for_response = ['移動源 C1','固定燃燒源 C1','工業製程 C1','人為逸散 C1(工時計算)','人為逸散 C1(冷媒設備)','人為逸散 C1(消防設備)','其他關注類物質 C1','輸入電力 C2(一般用電)','輸入電力 C2(綠電)','輸入蒸汽 C2(加項)','輸入蒸汽 C2(減項)']
    ...
    for i in range(len(source_type)):
        ...
        if source_type[i]!='人為逸散 C1' and source_type[i]!='輸入電力 C2' and source_type[i]!='輸入蒸汽 C2':
            ...
            excel_import(driver,source_type,file_path,upload_path,url,now,for_response)
            now+=1
            driver.get(url+'electricity')  #導回 電力頁面
...
def excel_import(driver,source_type,file_path,upload_path,url,now,for_response):
    ...
    if upload_path[now] not in {'Vehicle.xlsx', 'StationaryCombustionSource.xlsx', 'IndustrialProcess.xlsx', 'workhour.xlsx', 'other.xlsx', 'Elec.xlsx', 'GreenElec.xlsx', 'steam_plus.xlsx'}:
